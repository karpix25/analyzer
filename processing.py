...
def refine_crop_rect(
    ...
    def _detect_bottom_text_overlay(bgr_roi: np.ndarray) -> int:
        ...
        # УСЛОВНАЯ ОБРЕЗКА на основе ОДНОРОДНОСТИ снизу
        # Ключевая идея: текст/логотип обычно на ОДНОРОДНОМ фоне (черная/белая полоса)
        # Видео контент (песок, движение) - НЕОДНОРОДНЫЙ
        
        # Анализируем нижние 25% кадра
        bottom_height = int(h * 0.25)
        bottom_region = bgr_roi[h - bottom_height:, :]
        
        # Конвертируем в grayscale для анализа
        gray = cv2.cvtColor(bottom_region, cv2.COLOR_BGR2GRAY)
        
        # Вычисляем стандартное отклонение (мера однородности)
        bottom_std = gray.std()
        
        # РЕШЕНИЕ на основе однородности:
        # std < 15 → однородная область (вероятно полоса с текстом) → агрессивная обрезка
        # std >= 15 → неоднородная область (видео контент) → консервативная обрезка
        
        if bottom_std < 15:
            # Однородная область снизу → вероятно текст/логотип на полосе
            # Применяем агрессивную обрезку 40%
            trim = int(h * 0.40)
            logger.info(f"[TEXT_STD] Uniform bottom detected (std={bottom_std:.1f} < 15), applying 40% trim: {trim}px")
            return trim
        else:
            # Неоднородная область снизу → вероятно видео контент
            # Применяем консервативную обрезку 15%
            trim = int(h * 0.15)
            logger.info(f"[TEXT_STD] Non-uniform bottom (std={bottom_std:.1f} >= 15), applying 15% trim: {trim}px")
            return trim
